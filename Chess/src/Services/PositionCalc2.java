...
public class PositionCalc2 
{
	...
	public PositionCalc2(Position currentPosition)
	{
	...
	public List<Byte>  hasViewOf(byte thisPieceCoordinate,byte[] vision, boolean repeatable, boolean blocked)
	{
	...
	{
		PositionCalc2 newcalc= new PositionCalc2(position);
		...
		for(Map.Entry<Byte, Piece> entry : figurenDesGegners.entrySet())
		{
			if(entry.getValue() instanceof King)
			{
				...
				if(zugrecht)
				{
					pawnFelder = newcalc.hasViewOf(kingPosition,new byte[]{-7,-9} ,false,true);
				}
				else
				{
					pawnFelder = newcalc.hasViewOf(kingPosition,new byte[]{7,9} ,false,true);
				}
				List<Byte> kingFelder = newcalc.hasViewOf(kingPosition,new byte[]{ -7, 9, 7, -9,8, 1, 8, -1 } ,false,true);
				List<Byte> bishopFelder = newcalc.hasViewOf(kingPosition,new byte[]{ -7, 9, 7, -9 } ,true,true);
				List<Byte> rookFelder = newcalc.hasViewOf(kingPosition,new byte[]{8, 1, 8, -1} ,true,true);
				List<Byte> knightFelder = newcalc.hasViewOf(kingPosition,new byte[]{-15, -6, 10, 17, 15, 6, -10, -17 } ,false,true);
