...
public class Pawn extends Piece implements Copy {
	...
	public ArrayList<Byte> getMoves(Map<Byte, Piece> figurenAmZug, Map<Byte, Piece> figurenDesGegners,
		...
		if (super._color) {
			...
			if (!figurenDesGegners.containsKey((byte) (getCoordinate() - 8))
					&& !figurenAmZug.containsKey((byte) (getCoordinate() - 8))) {
				pieceFelder.add((byte) (getCoordinate() - 8));
				// Wenn auch vor diesem Feld nichs ist und der Bauer steht noch auf Startfeld
				if ((!figurenDesGegners.containsKey((byte) (getCoordinate() - 16))
						&& !figurenAmZug.containsKey((byte) (getCoordinate() - 16))) && getCoordinate() >= 48
						&& getCoordinate() <= 55) {
					pieceFelder.add((byte) (getCoordinate() - 16));
				}
			}
			// Wenn oben rechts ein gegner oder ein enPassant Feld ist
			if (figurenDesGegners.containsKey((byte) (getCoordinate() - 7))
					|| position.getEnPassant() == (byte) (getCoordinate() - 7)) {
			...
			if (figurenDesGegners.containsKey((byte) (getCoordinate() - 9))
					|| position.getEnPassant() == (byte) (getCoordinate() - 9)) {
		...
		// Wenn schwarz
			...
			if (!figurenDesGegners.containsKey((byte) (getCoordinate() + 8))
				...
				if (!figurenDesGegners.containsKey((byte) (getCoordinate() + 16))
						&& !figurenAmZug.containsKey((byte) (getCoordinate() + 16)) && getCoordinate() >= 8
						&& getCoordinate() <= 15) {
